...
for i=1:num_labels
     J = J + (1/m)*sum(transpose(-(y==i))*log(h(:,i)) -transpose((1 - (y==i)))*log(1 - h(:,i)));
end
...
tri_1 = transpose(del2)*X;
tri_2 = transpose(del3)*a2;

t1 = [zeros(size(Theta1, 1), 1) Theta1(:, 2:end)];
t2 = [zeros(size(Theta2, 1), 1) Theta2(:, 2:end)];
Theta1_grad = tri_1./m + (lambda/m)*(t1);
Theta2_grad = tri_2./m + (lambda/m)*(t2);
